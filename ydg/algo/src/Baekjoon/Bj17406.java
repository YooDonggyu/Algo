...
public class Bj17406 {
    ...
    public static int Rsum(int[][] copys) {
        ...
        return min;
			 ...
			 * 
			Deque<String> q ;
			int r, c, s, x, y, x1, y1, size, cycleCnt;
			while(cnt-- > 0) {
				conditions = br.readLine().split(" ");
				r= parseInt(conditions[0]);
				c= parseInt(conditions[1]);
				s= parseInt(conditions[2]);
				
				x = r-s; y = c-s; x1 = r+s; y1 = c+s; size = (x1-x+1); cycleCnt = size/2; 
				while(cycleCnt-->0) {
					//System.out.println("cycleCnt = "+cycleCnt);
					System.out.println("x = "+ x + ", y = "+ y +", x1 = "+ x1 + ", y1 = "+ y1);
					q = new ArrayDeque<String>();
					for(int i=y; i<=y1-1; i++) q.add(map[x][i]);
					for(int i=x; i<=x1-1; i++) q.add(map[i][y1]);
					for(int i=y1; i>=y+1; i--) q.add(map[x1][i]);
					for(int i=x1; i>=x+1; i--) q.add(map[i][y]);
					
					q.addFirst(q.removeLast()); // 한칸이동
										
					for(int i=y; i<=y1-1; i++) map[x][i] = q.removeFirst();
					for(int i=x; i<=x1-1; i++) map[i][y1] = q.removeFirst();
					for(int i=y1; i>=y+1; i--) map[x1][i] = q.removeFirst();
					for(int i=x1; i>=x+1; i--) map[i][y] = q.removeFirst();
					x++;
					y++;
					x1--;
					y1--;
